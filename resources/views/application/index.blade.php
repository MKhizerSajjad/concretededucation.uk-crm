                    ...
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        ...
                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                <li class=""><a href="{{ route('application.index') }}">Application</a></li>
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="d-flex justify-content-end gap-2" bis_skin_checked="1">
                                <a href="{{ route('application.create') }}" class="btn btn-primary waves-effect waves-light"> <i class="bx bx-plus me-1"></i> Add New</a>
                            </div>
                                        ...
                                        <tbody>
                                            @foreach ($data as $key => $app)
                                                <tr>
                                                    ...
                                                    <td>{{ $app->universirty->name }}</td>
                                                    <td>{{ $app->course->name }}</td>
                                                    <td>{{ $app->date_time }}</td>
                                                    <td>{{ $app->offer }}</td>
                                                    <td>{{ $app->interview }}</td>
                                                    <td>{{ $app->enroll_status }}</td>
                                                    <td>{{ $app->status }}</td>
                                                    <td class="text-center">
                                                        <a href="{{ route('case.invoice', $app->id) }}"><i class="bx bx-receipt"></i></a>
                                                        <a href="{{ route('case.edit', $app->id) }}"><i class="bx bx-pencil"></i></a>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
